...
from datasets.augmentor import SequenceDispFlowAugmentor, SequenceDispSparseFlowAugmentor
...
class StereoSequenceDataset(data.Dataset):
        ...
        for i in range(sample_size):
            for cam in ["left", "right"]:
                ...
                if "camera" in sample:
                    ...
                    # Sintel
                    if sample["camera"]["left"][0][-3:] == "cam" and cam in sample["camera"]:
                        f = open(sample["camera"]["left"][0], 'rb')
                        check = np.fromfile(f, dtype=np.float32, count=1)[0]
                        assert check == TAG_FLOAT, ' cam_read:: Wrong tag in flow file (should be: {0}, is: {1}). Big-endian machine? '.format(
                            TAG_FLOAT, check)
    ...
    def __getitem__(self, index):
        ...
        for k, v in output_tensor.items():
            if k != "viewpoint" and k != "metadata" and k != "depth2disp_scale" and k != "RTK" and k != "mask":
...
class DynamicReplicaDataset(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        root="/home/ywang/dataset/DynamicReplica/",
        split="train",
        subfile="None",
        sample_len=-1,
        ...
        for seq_name in seq_annot.keys():
            ...
            # if seq_name == self.subfile and self.subfile != "None": # a16a72-3_obj
                for cam in ["left", "right"]:
...
class InfinigenStereoVideoDataset(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        root="/home/ywang/dataset/Infinigen/",
        split="train",
        sample_len=-1,
        only_first_n_samples=-1,
    ):
        ...
        for i in range(len(sequence)):
            ...
            try:
                ...
                if self.split == "train" or "val" or "test":
                    for ref_idx in range(0, seq_len, 3):
                        step = 1 if self.sample_len == 1 else np.random.randint(1, 6)
                        if ref_idx + step * self.sample_len < seq_len:
                            sample = defaultdict(lambda: defaultdict(list))
...
class SouthKensingtonStereoVideoDataset(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        root="/home/ywang/dataset/SouthKensington/indoor/",
        split="test",
        subroot="",
        sample_len=-1,
        only_first_n_samples=-1,
...
class SouthKensingtonStereoVideoSubDataset(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        root="/home/ywang/dataset/SouthKensington/",
        split="test",
        dtype="indoor",
        subname="video010",
        sample_len=-1,
        ...
        super(SouthKensingtonStereoVideoSubDataset, self).__init__(aug_params)
        self.root = root
        self.split = split
        self.sample_len = sample_len
        ...
        filenames = defaultdict(lambda: defaultdict(list))
        for cam in ["left", "right"]:
            im_path_list = sorted(glob(osp.join(self.root, self.type, self.subname, "images", cam, "*.png")))
            if self.type == "indoor":
                camera_path = glob(osp.join(self.root, self.type, self.subname, "*.txt"))[0]
            for j in range(len(im_path_list)):
                im_path = im_path_list[j]
                assert os.path.isfile(im_path), im_path
                filenames["image"][cam].append(im_path)
                ...
                filenames["metadata"][cam].append([self.subname , (720,1280)])
                for k in filenames.keys():
                    assert (len(filenames[k][cam]) == len(filenames["image"][cam])
                                    > 0
                    ), self.subname
                            
        seq_len = len(filenames["image"][cam])
        print("seq_len", self.subname, seq_len)
...
class SequenceSceneFlowDataset(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        root="/home/ywang/dataset/SceneFlow/",
...
class SequenceSintelStereo(StereoSequenceDataset):
    def __init__(
        ...
        dstype="clean",
        subfile="None",
        root="/home/ywang/dataset/sintel_stereo/",
    ):
        super().__init__(
        ...
        self.dstype = dstype
        original_length = len(self.sample_list)
        image_root = osp.join(root, "training")
        ...
        for cam in ["left", "right"]:
            if subfile == "None":
                ...
                image_paths[cam] = sorted(
                glob(osp.join(image_root, f"{self.dstype}_{cam}/{subfile}"))
            )
...
class SequenceSintelStereo_train(StereoSequenceDataset):
    def __init__(
        self,
        aug_params=None,
        dstype="final",
        ...
        super().__init__(
            aug_params, reader=frame_utils.readDispSintelStereo
        )
        self.dstype = dstype
        self.sample_len = sample_len
        original_length = len(self.sample_list)
...
def fetch_dataloader(args):
    ...
    if add_dynamic_replica:
        dr_dataset_train = DynamicReplicaDataset(
            aug_params, split="train", sample_len=args.sample_len
        )
        dr_dataset_val = DynamicReplicaDataset(
            aug_params, split="valid", sample_len=args.sample_len
        )
        dr_dataset_test = DynamicReplicaDataset(
            aug_params, split="test", sample_len=args.sample_len
        )  
         
        dr_dataset_total = dr_dataset_train + dr_dataset_val
        logging.info(f"Adding {len(dr_dataset_total)} samples from DyanmicReplica")
    ...
    repeated_train_dataset = torch.utils.data.ConcatDataset([final_dataset] * 50 + [dr_dataset_total] * 50)
