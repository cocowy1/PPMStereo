...
class DynamicStereo(nn.Module):
    ...
    def forward_batch_test(
        self, batch_dict: Dict, kernel_size: int = 20, iters: int = 5
    ):
        ...
        for idx in range(len(predictions["disparity"])):
            plt.imsave(f'/home/ywang/my_projects/MemStereo/vis/sintel_final_dynamic/disp_{idx}.svg', predictions["disparity"][idx].squeeze().cpu().numpy(), vmin=2, vmax=83, cmap='magma')
        ############ add visualization ##############
    ...
    def plot_basis(self, feat):
        ...
        n, c, h, w=feat.shape
        n_components = 5
        feat = feat[0].squeeze().reshape(c, h*w).permute(1,0)
        feat = feat.detach().cpu().numpy()
        pca = PCA(n_components=n_components)
        pca.fit(feat)
        pca_features = pca.transform(feat).reshape(h,w, -1)
        for i in range(n_components):
            plt.imsave('/home/ywang/my_projects/dynamic_stereo/notebooks/pca_fea_{}.svg'.format(i), pca_features[:,:,i], cmap='jet')
        norm_feat = np.sqrt(np.sum(pca_features**2, axis=2))
        plt.imsave('/home/ywang/my_projects/dynamic_stereo/notebooks/norm_fea.svg'.format(i), norm_feat, cmap='jet')
    ...
    def forward(self, image1, image2, flow_init=None, iters=10, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision, dtype=torch.bfloat16):
            ...
            # self.plot_basis(fmap1)
            # x_norm = self.channel_length(fmap1)
            # image.imsave('/home/ywang/my_projects/dynamic_stereo/notebooks/fmap0.svg', x_norm[:, :, 2:, ...].detach().squeeze().cpu().numpy(), cmap='jet')
