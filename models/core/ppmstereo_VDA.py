...
from models.core.extractor import BasicEncoder, BasicEncoder_VFM, MultiBasicEncoder, MultiLevelEncoder_VFM, ResNetFPN, MultiLevelResNetFPN, conv3x3
...
class PPMStereo_VDA(nn.Module):
    def __init__(
        ...
        use_convex_3d: bool = True,
        init_flow: bool = False,
    ):
        super(PPMStereo_VDA, self).__init__()
        ...
        if self.use_vfm == True:
            self.fnet = MultiLevelEncoder_VFM(
            output_dim=dim, norm_fn="instance", dropout=self.dropout,
            )
            # from models.segment_anything.build_sam import Feature
            # self.model_size = "base"
            # self.backbone = Feature(self.model_size)

            from models.video_depth_anything.video_depth import VideoDepthAnything
            encoder='vits'
            model_configs = {
            ...
            self.backbone = VideoDepthAnything(**model_configs[encoder])
            self.backbone.load_state_dict(torch.load(f'./ckpt/vpt/vpt_{encoder}.pth', map_location='cpu'), strict=True)
            self.backbone = self.backbone.to('cuda').eval()

            
        else:
            self.fnet = MultiBasicEncoder(
            output_dim=dim, norm_fn="instance", dropout=self.dropout
        )
    ...
    def forward_batch_test(
        self, batch_dict: Dict, kernel_size: int = 20, iters: int = 10
    ):
    ...
    def forward(self, image1, image2, flow_init=None, iters=10, test_mode=False):
        ...
        if self.use_vfm:
            with torch.no_grad():
                vfm_features_list = self.backbone(torch.cat([image1, image2], dim=0), t=T)

        with autocast(enabled=self.mixed_precision, dtype=torch.bfloat16):
            if self.use_vfm == True:
                fmaps, fmaps_dw8, fmaps_dw16 = self.fnet([image1, image2], vfm_features_list)
            else:
                fmaps, fmaps_dw8, fmaps_dw16 = self.fnet([image1, image2])
            
            fmap1, fmap2 = fmaps[0], fmaps[1]
            fmap1_dw8, fmap2_dw8 = fmaps_dw8[0], fmaps_dw8[1]
            fmap1_dw16, fmap2_dw16 = fmaps_dw16[0], fmaps_dw16[1]
            ...
            if self.init_flow:
                ...
                flow_init_up = self.convex_upsample_3d(flow_init, weight_update, rate=4, T=T)
