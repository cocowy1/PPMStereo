...
> **GPU memory note:** Dynamic Replica uses high-resolution frames; a **≥48GB GPU** is recommended. If limited, set `MODEL.PPMStereoModel.kernel_size=10` (from 20), downscale the dataset, and/or reduce RAFT-style iterations (e.g., `iters=10` instead of 20) with a small accuracy drop.
