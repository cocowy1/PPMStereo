...
def eval_batch(batch_dict, predictions) -> Dict[str, Union[float, torch.Tensor]]:
    ...
    for idx in range(len(batch_dict["disparity"])):
        plt.imsave(f'/home/ywang/my_projects/MemStereo/vis/disp_gt/disp_{idx}.svg', batch_dict["disparity"][idx].squeeze().cpu().numpy(), vmin=2, vmax=83, cmap='magma')
