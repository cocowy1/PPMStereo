...
def sequence_loss(flow_preds, flow_gt, valid, uncertainties=None, flow_init_up=None, loss_gamma=0.9, max_flow=700):
    """Loss function defined over sequence of flow predictions"""
    flow_preds = flow_preds.float()
    ...
    for i in range(n_predictions):
        ...
        if uncertainties is not None:
            gt_ununcertainty = torch.exp(-0.9 *(flow_pred - flow_gt).abs()/7) + 1e-2
            i_loss = (flow_pred - flow_gt).abs() + (gt_ununcertainty - uncertainties[i]).abs()
        else:
    ...
    if flow_init_up is not None:
        assert (
            not torch.isnan(flow_init_up).any()
            and not torch.isinf(flow_init_up).any()
            )
        flow_pred = flow_init_up.clone()
        flow_loss += (flow_pred - flow_gt).abs()[valid.bool()].mean()
